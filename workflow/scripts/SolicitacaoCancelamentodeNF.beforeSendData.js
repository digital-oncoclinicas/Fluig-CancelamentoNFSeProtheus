function beforeSendData(customFields,customFacts){
	...
	//if(CURRENT_STATE == 5 || CURRENT_STATE == 6)
		//return;
	...
	var responsavel = hAPI.getCardValue("analyticsNmResponsavelSLA") == "" ? " " : hAPI.getCardValue("analyticsNmResponsavelSLA");
		...
		log.info("============beforeSendData  prazoSLA           : "+prazoSLA);
		
		
		if(prazoSLA == ""){
			log.info("============beforeSendData  prazoSLA em branco ");
			var c1 = DatasetFactory.createConstraint("metadata#active", "true", "true", ConstraintType.MUST);
			var c2 = DatasetFactory.createConstraint("codigo_sla", codigoSLA, codigoSLA, ConstraintType.SHOULD);
			var dataset = DatasetFactory.getDataset("ds_prazo_sla_form",null,new Array(c1,c2),null);
			
			hAPI.setCardValue("analyticsPrazoSLA",dataset.getValue(0,"prazo_sla"));
			hAPI.setCardValue("analyticsMedidaPrazoSLA",dataset.getValue(0,"medida_prazo"));
			prazoSLA = dataset.getValue(0,"prazo_sla");
		}
		...
		log.info("================================= beforeSendData Cancelamento de NF - fim "+ getValue("WKNumProces"));
